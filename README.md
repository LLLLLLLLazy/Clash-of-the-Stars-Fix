...
#### 8.7 建造落点与中后期产线再平衡（本次补充）
- `筑炼者.ini`：
  - `AI-FinalBuild`与`AI-FinalBuild-Late`新增“基地邻近+低威胁”约束（基地`withinRange=760`、敌军`withinRange=650`且`<2`）。
  - `终焉虫巢`建造目标由“当前位置”改为“己方最近基地”，避免终焉虫巢落在前线。
- `bug_spore.ini`、`bug_fly.ini`：
  - 提高矿区/占区建塔上限（自队与友队炮塔阈值上调），并适度降低触发资源门槛，加快前线防御成型。
  - 新增`AI-前线压进建塔`：中后段（`时点>=300`）在占领区遭遇敌军时主动补塔，强化前线据点防守。
- 低级单位中后期减产：
  - `飞虫地巢.ini`：`AI战斗飞虫征集`、`AIbuildbugFly`、`AIbuildbugBee`在`中刻{I}`后不再继续补量。
  - `飞虫地巢T2.ini`：`AIbuildAiraid`、`AIbuildAiraid空优`、`AI爆发高空压制`在`后界{I}`停用，防止后期继续刷低阶空军批次。
  - `虫巢.ini`：一号地巢开局低级兵批次由`6`下调为`4`，减少中后期低效兵线惯性。
